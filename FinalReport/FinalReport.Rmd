...
\noindent Next, we applied the SGD algorithm to the phishing dataset, carefully tuning the mini-batch size ($\alpha$) and learning rate ($\eta$) to optimize performance. We selected the optimal hyperparameters ($\alpha, \eta$) by evaluating both the convergence speed and the MSE on the training set. MSE was computed as the squared differences between the parameter estimates derived from our algorithm and those generated by the General Linear Model (GLM) function from the \textbf{stats} package in R. We chose a mini-batch size ($\alpha$) of 128 and a learning rate ($\eta$) of 0.01 for our final model. 
```{r, eval=FALSE}
